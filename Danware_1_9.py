...
def Hist_create(hist_name):
	...
	if "TH1" in hist_o.ClassName():
		...
		if cal_ans == "y":
			cut_ans = raw_input("Cut? [y / n] ")
			if cut_ans == "y": Cut(x_index, x_count)
...
def Medium_Back( i, j, original):
	a = original[j]
	b = ( original[j + i] + original[j - i] ) / 2.0
	c = 0
	a_i = i / 2
	c -= original[j - int(round(2*a_i))] / 6
	c += 4 * original[j - int(round(a_i))] / 6.0
	c += 4 * original[j + int(round(a_i))] / 6.0
	c -= original[j + int(round(2*a_i))] / 6.0

	if (b < c): b = c
	if (b < a): a = b
	return a

def Hard_Back( i, j, original):
	a = original[j]
	b = ( original[j + i] + original[j - i] ) / 2.0;
	...
	a_i = i / 2;
	c -= original[j - int(round(2*a_i))] / 6;
	c += 4 * original[j - int(round(a_i))] / 6;
	c += 4 * original[j + int(round(a_i))] / 6;
	c -= original[j + int(round(2*a_i))] / 6;

	d = 0
	a_i = i / 3
	d += original[j - int(round(3 * a_i))] / 20
	d -= 6 * original[j - int(round(2 * a_i))] / 20
	d += 15 * original[j - int(round(a_i))] / 20
	d += 15 * original[j + int(round(a_i))] / 20
	d -= 6 * original[j + int(round(2 * a_i))] / 20
	d += original[j + int(round(3 * a_i))] / 20
...
def Insane_Back( i, j, original):
	...
	e = 0
	a_i = i / 4
	e -= original[j - int(round(4 * a_i))] / 70
	e += 8 * original[j - int(round(3 * a_i))] / 70
	e -= 28 * original[j - int(round(2 * a_i))] / 70
	e += 56 * original[j - int(round(a_i))] / 70
	e += 56 * original[j + int(round(a_i))] / 70
	e -= 28 * original[j + int(round(2 * a_i))] / 70
	e += 8 * original[j + int(round(3 * a_i))] / 70
	e -= original[j + int(round(4 * a_i))] / 70
...
def B_Sub_Work(hist_name, it, b_num, count, opt):
	...
	Get_Data(hist_name, index, original)
	...
	if "b" in opt:
		...
		while i <= it:
			...
			if(i == 1): j = i 
			while j < hist_size - i:
				new_hist[j] = Basic_Back(i, j, original)
				j += 1
			j = i
			while j < hist_size - i:
	...
	if "m" in opt:
		...
		while i <= it:
			...
			if(i == 1): j = i 
			while j < hist_size - i:
				new_hist[j] = Medium_Back(i, j, original)
				j += 1
			j = i
			while j < hist_size - i:
				original[j] = new_hist[j]
	...
	if "h" in opt:
		...
		while i <= it:
			...
			if(i == 1): j = i 
			while j < hist_size - i:
				new_hist[j] = Hard_Back(i, j, original)
				j += 1
			j = i
			while j < hist_size - i:
				original[j] = new_hist[j]
	...
	if "i" in opt:
		...
		while i <= it:
			...
			if(i == 1): j = i 
			while j < hist_size - i:
				new_hist[j] = Insane_Back(i, j, original)
				j += 1
			j = i
			while j < hist_size - i:
				original[j] = new_hist[j]
	...
	k = 0 
	while k < hist_size:
		count.append(ref_hist[k] - original[k])
		b_num.append(index[k]) 
		k += 1
